...
# CLCO_PRD.md — 3차 UI 개편 보완 설계 (v3.2)

> **추가 작성**: 클코 (Cl-Co / 설계자)
> **기반**: 안티 (Anti / 기획자) 3차 지시서 v3.0 (2차 피드백 반영)
> **날짜**: 2026-05-30
> **수신**: 코덱스 (Codex / 전문 코더)

---

## A. v3.1 대비 v3.2 변경 요약

| 항목 | v3.1 | v3.2 (이번) |
|---|---|---|
| Hero 제거 / 로고 통일 / target 감사 | 설계 완료 | **동일, 재확인** |
| `post_process.py main()` 파이프라인 | `briefings` 리스트만 처리 | **`weekly`(17건) + `specials`(3건) 루프 추가** |

---

## B. 핵심 버그 진단 — `post_process.py main()` 누락
...
def main():
    ...
    for briefing in data.get("briefings", []):   # ← 일일 브리핑 148건만 처리
        ...
...
### B-2. 누락 데이터 실태 (briefings.json 실측)

| 리스트 키 | 건수 | date 형식 예시 | HTML 경로 |
|---|---|---|---|
| `briefings` | 148건 | `"2026-05-28"` | `archive/2026-05-28.html` |
| `weekly` | **17건** | `"weekly-2026-01-26"` | `archive/weekly-2026-01-26.html` ← 누락 |
| `specials` | **3건** | `"ces-2026-special"` | `archive/ces-2026-special.html` ← 누락 |

> `process_article()`이 호출되지 않은 20개 HTML 파일은 GNB와 푸터가 삽입되지 않은 상태로 방치.

---

## C. 설계 — `main()` 리팩토링

### C-1. 헬퍼 함수 `_process_list()` 신설

`main()` 함수 **바로 위**에 아래 헬퍼를 추가한다.
...
기존 `main()` 함수를 아래로 **완전히 대체**한다.

```python
def main():
    data = json.loads(BRIEFINGS_JSON.read_text("utf-8"))

    _process_list(data.get("briefings", []), "briefing")   # 일일 브리핑
    _process_list(data.get("weekly",    []), "weekly")     # 주간 브리핑 ← 신규
    _process_list(data.get("specials",  []), "special")    # 특별판      ← 신규
...
### Phase 1 — `index.html` Hero 완전 제거 (v3.1 동일)
- [ ] HTML: `<section class="hero">...</section>` 블록 삭제
- [ ] CSS: `.hero {}`, `.hero h1 {}`, `.hero p {}`, `.hero-stats {}`, `.stat {}` 5블록 삭제
- [ ] CSS 반응형: `@media` 내 `.hero` 관련 3개 규칙 삭제
- [ ] CSS: `.latest {}` → `padding: 2.5rem 2rem 3rem;` 교체

### Phase 2 — `index.html` Hero 비주얼 강화 (v3.1 동일)
- [ ] JS `renderLatest()` 커버 이미지 `w=800&h=450` → `w=1200&h=675` 교체
- [ ] CSS `.latest-card:hover` 보더 라이팅 강화 교체

### Phase 3 — GNB 로고 통일 (v3.1 동일)
- [ ] `archive.html` / `about.html` / `search.html` 로고 `Jae's Briefing` → `JFNB`

### Phase 4 — GNB `target="_blank"` 전수조사 (v3.1 동일)
- [ ] `grep -n 'target="_blank"'` 실행 → GNB 링크에 없음 확인

### Phase 5 — `post_process.py` 파이프라인 확장 ← **v3.2 신규 핵심**
- [ ] `_process_list()` 헬퍼 함수 `main()` 바로 위에 추가
- [ ] `main()` 전체를 C-2 코드로 대체
- [ ] `python briefing/scripts/post_process.py` 실행
- [ ] 로그에서 `[OK-weekly]`와 `[OK-special]` 항목이 출력되는지 확인
- [ ] `archive/weekly-2026-01-26.html` 브라우저 열어 GNB + 푸터 존재 확인
...
# Phase 5 검증
grep -n "_process_list\|weekly\|specials" briefing/scripts/post_process.py  # 모두 있어야 함
```

### 시각적 검증
- [ ] `index.html`: GNB 바로 아래 최신 브리핑 카드 등장, Hero 배너 없음
- [ ] `archive/weekly-2026-01-26.html`: 상단 GNB + 하단 푸터 정상 표시
- [ ] `archive/ces-2026-special.html`: 상단 GNB + 하단 푸터 정상 표시
- [ ] 모든 GNB 링크 동일 탭 전환

---

*v3.2 설계 완료 (2026-05-30). Phase 5가 이번 핵심 — weekly/specials HTML 후처리 누락 버그 봉합.*
